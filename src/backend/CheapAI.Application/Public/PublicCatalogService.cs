...
{
    public Task<PagedResult<PublicTestRecordListItemResponse>> GetLatestTestsAsync(int page, int pageSize, CancellationToken cancellationToken = default)
    {
        return publicCatalogRepository.GetLatestTestsAsync(page, Math.Clamp(pageSize, 1, 200), cancellationToken);
    }
